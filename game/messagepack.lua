...
local packers = setmetatable({  }, { __index = function(t, k)
...
packers['boolean'] = function(buffer, bool)
    if bool then
        buffer[#buffer + 1] = char(0xC3)
    else
        buffer[#buffer + 1] = char(0xC2)
    end
...
packers['string_compat'] = function(buffer, str)
    local n = #str
            if n <= 0x1F then
        buffer[#buffer + 1] = char(0xA0 + n)
    elseif n <= 0xFFFF then
        buffer[#buffer + 1] = char(0xDA, floor(n / 0x100), n % 0x100)
    elseif n <= 4294967295.0 then
        buffer[#buffer + 1] = char(0xDB, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
    else
...
packers['binary'] = function(buffer, str)
    local n = #str
            if n <= 0xFF then
        buffer[#buffer + 1] = char(0xC4, n)
    elseif n <= 0xFFFF then
        buffer[#buffer + 1] = char(0xC5, floor(n / 0x100), n % 0x100)
    elseif n <= 4294967295.0 then
        buffer[#buffer + 1] = char(0xC6, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
    else
...
packers['map'] = function(buffer, tbl, n)
            if n <= 0x0F then
        buffer[#buffer + 1] = char(0x80 + n)
    elseif n <= 0xFFFF then
        buffer[#buffer + 1] = char(0xDE, floor(n / 0x100), n % 0x100)
    elseif n <= 4294967295.0 then
        buffer[#buffer + 1] = char(0xDF, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
    else
...
packers['array'] = function(buffer, tbl, n)
            if n <= 0x0F then
        buffer[#buffer + 1] = char(0x90 + n)
    elseif n <= 0xFFFF then
        buffer[#buffer + 1] = char(0xDC, floor(n / 0x100), n % 0x100)
    elseif n <= 4294967295.0 then
        buffer[#buffer + 1] = char(0xDD, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
    else
...
packers['unsigned'] = function(buffer, n)
    if n >= 0 then
        ...
        elseif n <= 0xFF then
            buffer[#buffer + 1] = char(0xCC, n)
        elseif n <= 0xFFFF then
            buffer[#buffer + 1] = char(0xCD, floor(n / 0x100), n % 0x100)
        elseif n <= 4294967295.0 then
            buffer[#buffer + 1] = char(0xCE, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
        else
            buffer[#buffer + 1] = char(0xCF, 0, floor(n / 0x1000000000000) % 0x100, floor(n / 0x10000000000) % 0x100, floor(n / 0x100000000) % 0x100, floor(n / 0x1000000) % 0x100, floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
        end

    else
                                if n >= -0x20 then
            buffer[#buffer + 1] = char(0x100 + n)
        elseif n >= -0x80 then
            buffer[#buffer + 1] = char(0xD0, 0x100 + n)
        elseif n >= -0x8000 then
            n = 0x10000 + n
            buffer[#buffer + 1] = char(0xD1, floor(n / 0x100), n % 0x100)
        elseif n >= -0x80000000 then
            n = 4294967296.0 + n
            buffer[#buffer + 1] = char(0xD2, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
        else
            buffer[#buffer + 1] = char(0xD3, 0xFF, floor(n / 0x1000000000000) % 0x100, floor(n / 0x10000000000) % 0x100, floor(n / 0x100000000) % 0x100, floor(n / 0x1000000) % 0x100, floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
        end
...
packers['signed'] = function(buffer, n)
    if n >= 0 then
        ...
        elseif n <= 0x7FFFFFFF then
            ...
            buffer[#buffer + 1] = char(0xD3, 0, floor(n / 0x1000000000000) % 0x100, floor(n / 0x10000000000) % 0x100, floor(n / 0x100000000) % 0x100, floor(n / 0x1000000) % 0x100, floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100)
        end

    else
                                if n >= -0x20 then
            buffer[#buffer + 1] = char(0xE0 + 0x20 + n)
        elseif n >= -0x80 then
            buffer[#buffer + 1] = char(0xD0, 0x100 + n)
        elseif n >= -0x8000 then
            n = 0x10000 + n
...
packers['float'] = function(buffer, n)
    ...
    local mant, expo = frexp(n)
            if mant ~= mant then
        buffer[#buffer + 1] = char(0xCA, 0xFF, 0x88, 0x00, 0x00)
    elseif mant == huge or expo > 0x80 then
        if sign == 0 then
            buffer[#buffer + 1] = char(0xCA, 0x7F, 0x80, 0x00, 0x00)
        else
            buffer[#buffer + 1] = char(0xCA, 0xFF, 0x80, 0x00, 0x00)
        end

    elseif (mant == 0.0 and expo == 0) or expo < -0x7E then
        buffer[#buffer + 1] = char(0xCA, sign, 0x00, 0x00, 0x00)
    else
        expo = expo + 0x7E
...
packers['double'] = function(buffer, n)
    ...
    local mant, expo = frexp(n)
            if mant ~= mant then
        buffer[#buffer + 1] = char(0xCB, 0xFF, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
    elseif mant == huge or expo > 0x400 then
        if sign == 0 then
            buffer[#buffer + 1] = char(0xCB, 0x7F, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
        else
            buffer[#buffer + 1] = char(0xCB, 0xFF, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
        end

    elseif (mant == 0.0 and expo == 0) or expo < -0x3FE then
        buffer[#buffer + 1] = char(0xCB, sign, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
    else
        expo = expo + 0x3FE
...
packers['ext'] = function(buffer, tag, data)
    local n = #data
            if n <= 0xFF then
        buffer[#buffer + 1] = char(0xC7, n, tag < 0 and tag + 0x100 or tag)
    elseif n <= 0xFFFF then
        buffer[#buffer + 1] = char(0xC8, floor(n / 0x100), n % 0x100, tag < 0 and tag + 0x100 or tag)
    elseif n <= 4294967295.0 then
        buffer[#buffer + 1] = char(0xC9, floor(n / 0x1000000), floor(n / 0x10000) % 0x100, floor(n / 0x100) % 0x100, n % 0x100, tag < 0 and tag + 0x100 or tag)
    else
...
local unpackers = setmetatable({  }, { __index = function(t, k)
...
local function cursor_string(str)
    return { s = str, i = 1, j = #str, underflow = function(self)
        error "missing bytes"
    end }
end

local function cursor_loader(ld)
    return { s = '', i = 1, j = 0, underflow = function(self, e)
